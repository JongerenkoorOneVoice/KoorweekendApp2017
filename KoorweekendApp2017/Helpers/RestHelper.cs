	...
	public static class RestHelper
	{
		...
		public async static Task<T> GetRestDataFromUrl<T>(string requestUrl, BasicAuthentication authObj = null)
		{
			...
			if (network.InternetConnectionStatus() != NetworkStatus.NotReachable)
			{
				...
				if (authObj != null)
				{
					request.Headers["Authorization"] = authObj.HeaderValue;
				}
...
*/
		...
		public static T GetModelFromJson<T>(String RawData)
		{
			...
			// Extra check because for some webservices that return not an array but an object
			// the conversion below fails.

			if (rawDataDynamic.Type == JTokenType.Object)
			{
				var tempResult = TryDeserializeObject<T>(RawData);

				if (!tempResult.Equals(default(T))) return tempResult;
			}
			...
			return TryDeserializeObject<T>(temp);
